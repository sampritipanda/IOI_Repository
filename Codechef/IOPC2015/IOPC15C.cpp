...
// Taken from https://github.com/anudeep2011/programming/blob/master/qtree.cpp
...
long long baseArray[N], ptr;
long long chainNo, chainInd[N], chainHead[N], posInBase[N];
long long depth[N], pa[LN][N], otherEnd[N], subsize[N];
long long st[N*4], qt[N*4];
...
long long query_up(int u, int v) {
  if(u == v) return LLONG_MAX; // Trivial
  long long uchain, vchain = chainInd[v], ans = LLONG_MAX;
...
long long query(int u, int v) {
   ...
   */
  int lca = LCA(u, v);
  long long ans = query_up(u, lca); // One part of path
  long long temp = query_up(v, lca); // another part of path
